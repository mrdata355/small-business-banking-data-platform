...
def test_twin_is_deterministic_for_same_seed():
    a = BankingDigitalTwin(seed=99); a.seed_applications(50); ra = a.run()
    b = BankingDigitalTwin(seed=99); b.seed_applications(50); rb = b.run()
...
def test_faster_document_policy_improves_decision_cycle_time():
    ...
    assert result['median_time_to_decision_minutes']['candidate'] < result['median_time_to_decision_minutes']['baseline']
...
def test_contract_genome_flags_business_key_change_and_lineage_blast_radius():
    ...
    proposed = replace(previous, version='2.0.0', business_keys=('application_id', 'event_version'))

...
class ApplicationStateMachine:
    """Deterministic canonical-state reducer with replay and stale-version protection."""
    ...
    def reduce_many(self, application_id: str, events: Iterable[ApplicationEvent]) -> ApplicationState:
        ...
        for event in sorted(events, key=lambda e: (e.event_version, e.event_ts, e.event_id)):
    ...
    def _apply(self, state: ApplicationState, event: ApplicationEvent) -> ApplicationState:
        ...
        if event.event_type == "LoanApplicationSubmitted":
            ...
            changes["financial_package_complete"] = bool(payload.get("financial_package_complete", True))
            changes["status"] = ApplicationStatus.REVIEW
        elif event.event_type == "IdentityVerificationUpdated":
            changes["identity_status"] = IdentityStatus(str(payload.get("identity_status", "VERIFIED")))
            ...
            changes["funded_amount"] = float(amount) if amount is not None else state.funded_amount
...
def parse_event(record: Mapping[str, object]) -> ApplicationEvent:
    ...
    reserved = {"event_id", "application_id", "event_type", "event_version", "event_ts", "source_system"}
